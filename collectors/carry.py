...
def report(top=15):
    ...
    if rates is not None:
        try:
            rr, _ = rates.all_rates()
            sofr = rr.get("SOFR")
            if sofr:
                print(f"\nOFFCHAIN BENCHMARK: SOFR {sofr['value']:.2f}% ({sofr['date']})")
                btc = fund.get("BTC", {}).get("lighter")
                if btc is not None:
                    print(f"  BTC perp funding {btc*100:.2f}% vs SOFR {sofr['value']:.2f}%: "
                          f"spread {(btc*100 - sofr['value']):+.2f}pp over the offchain cost of money")
        except Exception as e:
            print(f"\n(SOFR unavailable: {type(e).__name__})")
